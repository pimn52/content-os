...
type Draft = { project_id: UUID; version: number; script: string | null; topic: string | null; scenes: ScenePlan[]; routes: Route[]; confirmed: Candidate[]; video_spec: VideoSpec | null };
...
function App() {
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try { const value = await api<IPProfile>("/ip-profile", { method: "PUT", body: JSON.stringify({ creator_name: String(form.get("creator_name") ?? ""), audience: String(form.get("audience") ?? "") || null, domains: split(String(form.get("domains") ?? "")), topics: split(String(form.get("topics") ?? "")), knowledge: split(String(form.get("knowledge") ?? "")), opinions: split(String(form.get("opinions") ?? "")), style_notes: split(String(form.get("style_notes") ?? "")), boundaries: split(String(form.get("boundaries") ?? "")), vocabulary: profile.vocabulary, avoided_expressions: profile.avoided_expressions, common_hooks: profile.common_hooks, metadata: profile.metadata }) }); setProfile(value); setMessage({ text: "IP 资料已保存并生成新版本" }); } catch (error) { setMessage({ text: error instanceof Error ? error.message : "保存失败", error: true }); } finally { setBusy(false); }
  }
  ...
  async function planProject() {
    ...
    try {
      ...
      const result = await api<{ scenes: ScenePlan[] }>(`/projects/${selectedProject.id}/scene-plan`, { method: "POST", body: JSON.stringify({ script: script || undefined, topic }) });
      ...
      await persistDraft(result.scenes, [], null, [], script, topic);
      setMessage({ text: `已生成 ${result.scenes.length} 个场景并保存草稿；下一步查找素材` });
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="card"><div className="section-heading"><div><span className="kicker">04 / DRAFT</span><h2>项目与草稿</h2></div><span className="badge">可恢复</span></div><div className="project-toolbar"><form onSubmit={createProject} className="project-form"><Field label="项目标题" name="title" placeholder="本次内容的名称" /><Field label="主题" name="topic" placeholder="可以只填主题，不必先写完整稿件" /><button disabled={busy} className="button primary">创建项目</button></form><label className="project-select">当前项目<select value={selectedProjectId ?? ""} onChange={(event) => setSelectedProjectId(event.target.value || null)}><option value="">选择项目</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title} · {project.topic}</option>)}</select></label></div>
        {selectedProject && <div className="production"><div className="row-between"><div><h3>{selectedProject.title}</h3><p className="muted">{selectedProject.topic} · 草稿 v{draft?.version ?? 0}</p></div><div className="actions"><button disabled={busy} className="button" onClick={() => void planProject()}>生成 ScenePlan</button><button disabled={busy || !draft?.scenes.length} className="button" onClick={() => void routeProject()}>查找候选</button><button disabled={busy || !routes.length} className="button" onClick={() => void assembleSourceLedProject()}>按原声 + ASR 组装</button><button disabled={busy || !routes.length} className="button" onClick={() => void assembleProject()}>新旁白组装</button><button disabled={busy || !videoSpec} className="button primary" onClick={() => void renderProject()}>本地渲染</button></div></div>
        ...
        {draft && <div className="draft-editor"><label>脚本 / 旁白稿（可选）<textarea value={draft.script ?? ""} onChange={(event) => updateDraftScript(event.target.value)} placeholder="可以先写要表达的观点；留空时只按主题规划。" /></label><button disabled={busy} className="button" onClick={() => void saveDraftText()}>保存稿件</button><small className="muted">保存后刷新或切换项目仍会保留；编辑脚本或场景文案会先使旧计划、候选和渲染失效。</small></div>}
