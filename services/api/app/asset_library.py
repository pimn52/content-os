...
<div class="toolbar"><input id="query" aria-label="搜索素材" placeholder="例如：本人坐在电脑前操作软件"><input id="project-id" aria-label="项目 ID（模型语义搜索需要）" placeholder="项目 ID（模型语义搜索需要预算归属）"><button id="search">语义搜索</button><button id="reload">刷新素材</button></div>
<p class="muted">本地词法搜索不需要项目；调用模型的语义搜索必须关联一个项目，才能在该项目与全局预算中留痕。</p>
...
async function search(){const root=document.querySelector('#results');const query=document.querySelector('#query').value.trim();const projectId=document.querySelector('#project-id').value.trim();if(!query){root.innerHTML='<p class="error">请输入搜索内容。</p>';return}root.textContent='正在搜索…';try{const body={query,top_k:10};if(projectId)body.project_id=projectId;const response=await fetch('/clips/search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const result=await response.json();if(!response.ok)throw new Error(result.detail||'搜索失败');root.innerHTML=`<h2>搜索结果</h2>`+(result.map(h=>`<article class="result"><strong>${esc(h.clip.transcript||h.clip.visual_description||'未命名片段')}</strong><p>${fmt(h.clip.start_ms)} – ${fmt(h.clip.end_ms)} · 匹配度 ${Number(h.score).toFixed(3)}</p><p class="tags">${esc(tags(h.clip))}</p><a href="/clips/${h.clip.id}/media" target="_blank" rel="noopener">打开原视频</a></article>`).join('')||'<p>没有匹配片段。</p>')}catch(error){root.innerHTML=`<p class="error">${esc(error.message)}</p>`}}
