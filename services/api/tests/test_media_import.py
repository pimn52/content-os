...
from app.db import AssetRepository, CURRENT_SCHEMA_VERSION, Database, apply_migrations
...
def test_migration_from_v1_backfills_asset_hash_and_unique_index(tmp_path: Path):
    ...
    assert [row[0] for row in connection.execute("SELECT version FROM schema_migrations ORDER BY version")] == list(range(1, CURRENT_SCHEMA_VERSION + 1))
